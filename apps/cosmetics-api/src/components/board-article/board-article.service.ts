...
export class BoardArticleService {
  ...
  public async getBoardArticle(
    ...
    const targetBoardArticle: BoardArticle | null = await this.boardArticleModel
      .findOne(search)
      .lean()
      .exec(); // lean modify qilish uchun ishlatyapmiz
  ...
  public async getBoardArticles(
    ...
    const sort: T = {
      [input?.sort ?? 'createdAt']:
        (input?.direction ?? Direction.ASC) ? 1 : -1, // ✅ convert ASC/DESC to 1/-1
    };
  ...
  public async likeTargetBoardArticle(
    ...
    if (modifier > 0 && String(target.memberId) !== String(memberId)) {
      await this.notificationService.createSystemNotification({
        ...
        notificationDesc: 'A user liked your board article.',
        articleId: likeRefId,
      });
    }
  ...
  public async getAllBoardArticlesByAdmin(
    ...
    const sort: T = {
      [input?.sort ?? 'createdAt']:
        (input?.direction ?? Direction.DESC) ? 1 : -1,
    };
